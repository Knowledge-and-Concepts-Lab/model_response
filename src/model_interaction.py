...
def get_responses(batches, model_type, temperature, output_path, batch_size=256, max_tokens=256):
    if model_type not in model_list:
        print(f"Error: Please select from following models: {model_list}")
        exit()
        
    if model_type in gpts:
        return generate_responses_gpt(batches, model_type, output_path, temperature, max_tokens)
